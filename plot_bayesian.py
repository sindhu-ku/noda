...
# First create Inference Data for arviz
stat_opt = sys.argv[1]
show = 0
...
file_list = []
nwalkers = 30
for i in range(481, 497):
    file_list.append(f"npz_files/MCMC_Bayesian_1_{nsteps}_{i}_{stat_opt}.npz")
...
if show == 1 : plt.show()
else : plt.savefig(f"Plots/{stat_opt}/autocorrectplot.png")
...
for i in range(nvars):
    ...
    if show ==1 : plt.show()
    else : plt.savefig(f'Plots/{stat_opt}_{nsteps}events/stepnumber.png')



if show ==1 : plt.show()
else : plt.savefig(f'Plots/{stat_opt}_{nsteps}events/stepnumber-what.png')

npchains_aw = npchains.reshape(nfiles*nwalkers,nsteps,nvars)

chainr_aw  = range(nfiles*nwalkers)
...
if show ==1 : plt.show()
else : plt.savefig(f'Plots/{stat_opt}_{nsteps}events/az.png')

npchains_cut  = npchains[:,skipsteps:,:]
npchains_full = npchains_cut.reshape(nfiles*nwalkers*(nsteps-skipsteps),nvars)
figure = cr.corner( npchains_full , labels = labels )

if show ==1 : plt.show()
else : plt.savefig(f'Plots/{stat_opt}_{nsteps}events/corner.png')
